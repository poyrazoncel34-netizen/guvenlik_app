  ...
  group('No SMS imports in lib/', () {
    test('no file imports sms_service.dart', () {
      final offenders = dartFiles()
          .where((f) => f.readAsStringSync().contains("'sms_service.dart'") ||
    ...
    test('no file imports emergency_orchestrator.dart', () {
      final offenders = dartFiles()
          .where((f) =>
              f.readAsStringSync().contains('emergency_orchestrator.dart'))
          .map((f) => f.path)
          .toList();
      expect(offenders, isEmpty,
          reason:
