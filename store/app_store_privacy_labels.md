# App Store — Privacy Nutrition Labels

Bu belge, App Store Connect'te "Uygulama Gizliliği" bölümü için hazır yanıtları içermektedir.

Son güncelleme: 18 Mart 2026

---

## Data Used to Track You (Sizi Takip Etmek İçin Kullanılan Veriler)

**YOK** — Bu uygulama sizi izlemez, reklam profillemesi yapmaz.

---

## Data Linked to You (Sizinle İlişkilendirilen Veriler)

| Kategori | Alt Tür | Kullanım Amaçı |
|----------|---------|----------------|
| **Contact Info** | Name (Ad) | App Functionality |
| **Contact Info** | Email Address | App Functionality |
| **Photos or Videos** | Photos | App Functionality |
| **Location** | Precise Location | App Functionality |
| **Contacts** | Contacts | App Functionality |

---

## Data Not Linked to You (Sizinle İlişkilendirilmeyen Veriler)

| Kategori | Alt Tür | Kullanım Amaçı |
|----------|---------|----------------|
| **Audio** | Other Audio Data | App Functionality |
| **Identifiers** | Device ID | App Functionality |

---

## Notlar

1. **Konum**: Yalnızca acil durum tetiklendiğinde ve kullanıcı açıkça izin verdiğinde toplanır. Sunucuya gönderilmez.
2. **Biyometrik**: Bu sürümde biyometrik kilit kullanılmaz; uygulama biyometrik veriyi işlemez.
3. **Ses**: Kullanıcı başlatır, yalnızca cihazda saklanır. İCloud'a veya sunucuya gönderilmez.
4. **Kişiler**: Yalnızca kullanıcının seçtiği kişiler saklanır. Rehber taranmaz.

---

## App Store Connect Form Doldurma Kılavuzu

### "Does your app collect data?" sorusu:
✅ **Yes**

### "Do you or your third-party partners use the data from this app for any purpose other than providing or improving the features of your app?" sorusu:
❌ **No**

### Gizlilik Politikası URL:
`https://poyrazoncel34-netizen.github.io/korubeni/privacy`

---

## Özet (Kullanıcıya Gösterilen)

```
KoruBeni veri gizliliği uygulamaları:
- Verileriniz yalnızca cihazınızda saklanır
- Üçüncü taraflarla paylaşılmaz
- Reklam amacıyla kullanılmaz
- İstediğiniz zaman tüm verilerinizi silebilirsiniz
```
