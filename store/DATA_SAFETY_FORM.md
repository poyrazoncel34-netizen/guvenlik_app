...
| Phone number | Not collected by developer backend | Emergency contact numbers are stored locally. While a safety session is armed, a normalized target is kept in Android device-protected app storage for reboot/locked-boot recovery and may be passed to Android Telecom at expiry. A fallback target is retained app-private for at most 24 hours and removed on action/dismiss/TTL. |
...
| Anonymous app/device/subscription identifiers and IP address (connection-level) | Processed by RevenueCat / Google Play Billing where applicable | RevenueCat is lazily configured only after current legal acceptance and a Pro/paywall/restore or prior-Pro-hint path. Used for purchase verification, entitlement status, fraud prevention, and restore. The prior-Pro hint is local and is not entitlement proof. |
...
```text
KoruBeni does not operate a developer backend. Emergency contacts, profile data, PIN, fake-call settings, local activity history, and consent records are stored on device. The app does not read the full contacts list; it stores only emergency contacts selected or entered by the user. An armed safety session keeps only a minimal device-protected token/deadline/normalized-target envelope for reboot recovery; a fallback target is app-private for at most 24 hours and is removed on action, dismiss, or TTL. Optional Pro subscription processing is handled by Google Play Billing and RevenueCat, which is configured lazily after current legal acceptance. Online map screens may request map tiles from the configured map tile provider, currently OpenStreetMap tile infrastructure, only for the map viewport the user actively opens; this can expose IP, User-Agent, and requested tile coordinates. Default-provider tiles are kept in an app-private bounded cache according to HTTP cache directives, or for seven days when no usable directive is supplied, and are removed with app-data reset/uninstall. The app does not bulk download, scrape, pre-seed, create offline packs, or package OpenStreetMap public tiles. The app does not use ads, analytics, remote crash reporting SDKs, auth backend, cloud database, SMS sending, microphone, audio recording, account creation, or user-generated content in this Play release.
```
