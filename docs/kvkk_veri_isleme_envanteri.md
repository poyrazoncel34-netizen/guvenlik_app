...
**Son teknik güncelleme:** 18 Temmuz 2026
...
| Harita request metası: IP, User-Agent, tile z/x/y; viewport çıkarımı | Cihaz ağı ve kullanıcının açtığı map viewport'u | Çevrimiçi harita açıldığında | Harita karosu indirme | Hizmet/consent ve m.9 şartı counsel tarafından belirlenir | Yapılandırılmış provider; ilk sürümde public OSM endpoint adayı | Yurt dışı aktarım ihtimali yüksek; ülke ve mekanizma `UNVERIFIED` | Provider log politikası; app cache en az 7 gün hedefi, prefetch/offline archive yok | App cache temizleme; provider tarafı silme doğrudan garanti edilmez | Network capture, stable UA/contact, attribution, cache-header/backoff testi | `COUNSEL_UNVERIFIED`, `NETWORK_EVIDENCE_REQUIRED` |
