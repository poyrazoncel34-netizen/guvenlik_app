...
# gorunur kilar. Yalniz strict modda: ~4 dk surer ve ag ister, gunluk dogrulama
# dongusu bu maliyeti tasimamali.
#
# Hedef neden :app:assemblePlayDebug: ilk surumde hedef
# :app:checkPlayDebugAarMetadata idi ve KAPSAM BOSLUGU birakti — CI'daki ikinci
# kirmizi :audioplayers_android:compileDebugLibraryResources'ten geldi (aapt2).
# Eklenti modullerinin kendi konfigurasyonlari :app metadata kontroluyle
# cozulmez. assemblePlayDebug her eklenti modulunun kaynak derlemesini de
# tetikledigi icin CI'in cozdugu kumeye cok daha yakindir.
#
# Bu kapinin YAKALAYAMADIGI sinif: host-OS'a bagli siniflandirmali artefaktlar
# (ornegin aapt2 -linux vs -osx). macOS'ta --refresh-dependencies bile linux
# siniflandirmasini cozmez. O eksen statik testle kapatilir:
# test/release_hardening_contract_test.dart -> 'platform-classified artifacts'.
if [ "$STRICT_RELEASE_GATES" -eq 1 ]; then
  ...
  if [ -n "${CI:-}" ]; then
    ...
    if [ "$DEPVERIFY_STATUS" -eq 0 ]; then
      ...
      printf '              --refresh-dependencies :app:assemblePlayDebug\n'
      printf '    Girdiyi ELLE yazma: provenans (origin="Generated by Gradle") bozulur.\n'
      printf '    Eksik olan host-OS siniflandirmali bir artefaktsa (ornegin\n'
      printf '    aapt2 -linux), yazici o siniflandirmayi bu makinede cozemez;\n'
      printf '    gecici bir init script ile cozdurup urettirilmelidir.\n'
      rm -f "$DEPVERIFY_LOG"
